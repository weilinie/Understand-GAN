...
from img_helpers import generate_gmm_circle_data, batch_gmm_gen
...
class GAN_model(object):
    def __init__(self, config):
        ...
        self.img_dim = config.img_dim_gmm
    ...
    def build_model(self):
        ...
        x = tf.reshape(self.x, [-1, 1, self.img_dim, 1])
        img_chs = x.get_shape().as_list()[-1]

        self.gmm_data, self.means = generate_gmm_circle_data(num_cluster=self.num_cluster)

        # x = x_img / 127.5 - 1. # Normalization
        ...
        d_out_fake, _ = discriminator(
            self.d_net, self.fake_data, self.conv_hidden_num,
            self.normalize_d
        )

        self.d_loss, self.g_loss, metric = self.cal_losses(
            x, self.fake_data, d_out_real, d_out_fake, self.loss_type
        )
    ...
    def train(self):
        ...
        gmm_gen = batch_gmm_gen(self.gmm_data, self.batch_size)
        ...
        with self.sv.managed_session() as sess:
            for _ in range(self.max_step):
                ...
                if step % self.save_step == 0:
                    self.generate_samples(sess, self.means, z_fixed, self.model_dir, idx=step)

    def generate_samples(self, sess, means, z_fixed, model_dir, idx):
        ...
        save_gmm(x, path,
                 center=means, title="Minibatch: " + str(idx),
                 is_color=True)
    ...
    def interpolate_G(self, sess, means, save_dir, test_size=20):
        z1 = np.random.normal(size=[test_size, self.z_dim])
        z2 = np.random.normal(size=[test_size, self.z_dim])
        generated = []
        for idx, ratio in enumerate(np.linspace(0, 1, 100)):
        ...
        for idx, img in enumerate(generated):
            path = os.path.join(save_dir, 'sample_G_{:06d}.png'.format(idx))
            save_gmm(img, path,
                     center=means, title="test_interp: " + str(idx),
                     is_color=True)

        all_img_num = np.prod(generated.shape[:2])
        batch_generated = np.reshape(generated, [all_img_num] + list(generated.shape[2:]))
        path = os.path.join(save_dir, 'test_interp_G.png')
        save_gmm(batch_generated, path,
                 center=means, title="test_interp",
                 is_color=True)

    def one_hot_input_viz(self, sess, save_dir, means):
        z = np.eye(self.z_dim, self.z_dim)
        generator_bias_x = sess.run(self.samples_test, {self.z_test: np.zeros_like(z)})
        print('generator_bias_x: {}'.format(generator_bias_x[0, 0]))
        save_gmm(generator_bias_x, os.path.join(save_dir, 'test_bias.png'),
                 center=means, title="generator_bias_x", is_color=True)

        for i in range(1, 101, 10):
            z_var = z*i

            samples_OH = sess.run(self.samples_test, {self.z_test: z_var})
            samples_columns = samples_OH - generator_bias_x
            print('samples_columns: {}'.format(samples_columns[0, 0]))
            save_gmm(samples_OH, os.path.join(save_dir, 'test_one_hot_input_add_bias_{}.png'.format(i)),
                     center=means, title="sample_OH")
            save_gmm(samples_columns, os.path.join(save_dir, 'test_one_hot_input_{}.png'.format(i)),
                     center=means, title="samples_columns")


    def test(self):
        print('start testing...')
        with self.sv.managed_session() as sess:
            self.interpolate_G(sess, means=self.means, save_dir=self.test_model_dir)

            self.one_hot_input_viz(sess, means=self.means, save_dir=self.test_model_dir)
